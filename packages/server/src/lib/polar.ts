...
export function getPolarServer(): PolarServer {
  ...
  if (server !== 'sandbox' && server !== 'production') {
    throw new Error(`POLAR_SERVER must be either "sandbox" or "production", got: ${server}`);
  }
  return server;
}
...
export async function getAvailableCreditsBalance(customerExternalId: string) {
  try {
    ...
    if (matchingMeters.length > 1) {
      throw new Error(
        'Expected only one matching meter for the credits meter ID, but found multiple.',
      );
    }
